...
```bash
kubectl get pods --namespace=pedidos-ms
```
...
```bash
  kubectl exec -it --namespace=pedidos-ms POD_API --  sh /var/bin/migration.sh
```
...
```bash
kubectl --namespace=pedidos-ms cp seeder.sql POD_POSTGRES:/tmp
```

Executar o seed

```bash
kubectl exec -it --namespace=pedidos-ms POD_POSTGRES --  psql -U pedidos-ms -d pedidos-ms -f /tmp/seeder.sql
```
